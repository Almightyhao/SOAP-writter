...
此欄位僅供製圖使用。**請一行只放一個** Lab 項目或藥物。

#### 1. Lab 趨勢圖 (折線圖)
* **支援關鍵字：** `SCr`, `WBC`, `K+`, `Na+`, `Plt`, `AST`, `ALT`, `Hb`, `BUN`, `CRP`
* **格式：** 爬蟲會自動抓取「數值」和「日期」的配對，並自動忽略中間的單位或文字。
* **範例：**
    ```
    SCr: 1.2 mg/dl (10/1), 1.5 (10/2) -> 0.9 (10/3)
    WBC: 12.5 K/uL (10/1) 15.0 (10/2) 11.0 (10/3)
    K+: 3.5 (10-1) 4.0(10-2) 3.8(10-3)
    AST: 80(114/10/1) 120(114/10/2) 90(114/10/3)
    ```

#### 2. 用藥史 (甘特圖)
* **支援關鍵字：** 任何藥物名稱 (例如 `Vanco`, `Mero`, `Tazocin`)。
* **格式：** `[藥物名稱]: ([起始日]-[結束日])`。日期格式同樣支援彈性解析。
* **範例：**
    ```
    Vanco: (10/1-10/5)
    Mero: (10/3-10/7)
    Tazocin: (114/10/5 - 114/10/10)
    Levaquin: (10/8-10/12)
    ```

*(備註：Lab 數據必須至少有兩個點才能繪製成線圖)*
